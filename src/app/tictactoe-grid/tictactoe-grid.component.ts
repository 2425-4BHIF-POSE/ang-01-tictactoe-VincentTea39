...
export class TictactoeGridComponent {
  ...
  public readonly board: WritableSignal<Player>[] = Array(this.fieldCount.length).fill(signal(this.Player.none));
  ...
  public draw(index: number) {
    this.board[index].set(this.CurrentPlayer);
  ...
  public restartGame() {
    this.board.fill(signal(this.Player.none));
  }
